...
4. **精排**：逐篇清理噪音、恢复标题层级、统一排版，PUT 写回。精排规则：
   - 删除导航栏、侧边栏、广告、相关推荐、平台引流链接、版权声明等非文章内容
   - 识别 HTML `<pre><code>` 代码块，转为 Markdown 三个反引号围栏，确保围栏配对完整
   - 代码块之后用空行分隔，防止 Obsidian 将后续正文误解为代码
   - 保留原文信息：原文链接、作者、发布日期
